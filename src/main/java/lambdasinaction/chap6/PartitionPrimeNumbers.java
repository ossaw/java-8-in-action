...
public class PartitionPrimeNumbers {
    ...
    public static Map<Boolean, List<Integer>> partitionPrimes(int n) {
        return IntStream.rangeClosed(2, n).boxed().collect(partitioningBy(
                candidate -> isPrime(candidate)));
    }

    public static boolean isPrime(int candidate) {
        return IntStream.rangeClosed(2, candidate - 1).limit((long) Math.floor(
                Math.sqrt((double) candidate)) - 1).noneMatch(i -> candidate
                        % i == 0);
    }
    ...
    public static boolean isPrime(List<Integer> primes, Integer candidate) {
        ...
        return false;
        // TODO error, To be corrected
        /*return primes.stream().takeWhile(i -> i <= candidateRoot).noneMatch(
                i -> candidate % i == 0);*/
    }
    ...
    public Map<Boolean, List<Integer>> partitionPrimesWithInlineCollector(
            int n) {
